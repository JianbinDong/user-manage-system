...
class UserServiceImpl extends BaseService implements UserService
{
     ...
     * roster表
     */
    public function getRoster($userId)
    {
        $basic = $this->getDao('user_basic_dao')->getByUserId($userId);
        $user = $this->getUser($userId);
        $roster = $this->getDao('roster_dao')->getByUserId($userId);
        $basic = array_merge($user, $basic);
        if (empty($roster)) {
            $roster = $basic;
        } else {
            $roster = array_merge($basic, $roster);
        }
        
        return $roster;
    }
    
    public function createRoster($fields)
    {
        $tableFields = $this->getDao('roster_dao')->getTableFields();
        $fields = ArrayToolkit::parts($fields, $tableFields);
        unset($fields['id']);

        return $this->getDao('roster_dao')->create($fields);
    }

    public function updateRoster($userId, $fields)
    {
        $basic = $this->getDao('user_basic_dao')->getByUserId($userId);
        ...
        $tableFields = $this->getDao('roster_dao')->getTableFields();
        $fields = ArrayToolkit::parts($fields, $tableFields);
        return $this->getDao('roster_dao')->update($roster['id'], $fields);
    }
    ...
    {
        return $this->getDao('user_dao')->update($userId, $fields);
    }

    public function getUser($userId)
    {
        return $this->getDao('user_dao')->get($userId);
    }
    ...
    {
        return $this->getDao('user_dao')->getByNumber($number);
    }

    public function searchUsers($conditions, $order, $start, $limit)
    {
        return $this->getDao('user_dao')->search($conditions, $order, $start, $limit);
    }
    ...
    {
        $basic = $this->getDao('user_basic_dao')->getByUserId($userId);
        $user = $this->getDao('user_dao')->get($userId);    
        $basic['joinTime'] = $user['joinTime'];
