...
use Biz\Verify\Impl\VerifyServiceImpl;
...
use Biz\Verify\Dao\Impl\VerifyDaoImpl;
use Biz\User\Dao\Impl\RosterDaoImpl;
...
class BizKernel extends Kernel
{
    ...
    protected function registerService()
    {
        ...
        $this['verify_dao'] = $this->dao(function($container) {
            return new VerifyDaoImpl($container);
        });

        $this['roster_dao'] = $this->dao(function($container) {
            return new RosterDaoImpl($container);
        });
        
        $this['verify_service'] = function($container) {
            return new VerifyServiceImpl($container);
        };
